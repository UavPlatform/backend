...
public interface UavRepository extends JpaRepository<Uav, Long> {
    ...
    @Query("SELECT new com.uav.uav.pojo.vo.UavVo(u.id, u.uavName, u.djiId, u.controllerModel, u.isAvailable) FROM Uav u WHERE u.uavName = :name")
    Optional<UavVo> findUavByUavName(@Param("name") String uavName);

    @Query("SELECT new com.uav.uav.pojo.vo.UavVo(u.id, u.uavName, u.djiId, u.controllerModel, u.isAvailable) FROM Uav u")
    List<UavVo> getAll();

    @Query("SELECT new com.uav.uav.pojo.vo.UavVo(u.id, u.uavName, u.djiId, u.controllerModel, u.isAvailable) FROM Uav u WHERE u.onlineStatus = :status")
